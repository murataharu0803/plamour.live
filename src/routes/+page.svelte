...
const gridHeight = Math.max(...crosswordData.map(d => d.direction === "down" ? d.row + d.word.length : d.row)) + 1
const gridWidth = Math.max(...crosswordData.map(d => d.direction === "across" ? d.col + d.word.length : d.col)) + 1
...
const onKeyDown = (e: KeyboardEvent) => {
  if (e.key === 'ArrowUp') {
    ...
    const currentWord = crosswordData[currentWordIndex]
    if (currentWord.direction === "across" && currentCell.col > currentWord.col) currentCell.col--
    else if (currentWord.direction === "down" && currentCell.row > currentWord.row) currentCell.row--
...
<div use:onload class="relative">
  {#each answerGrid as row, i}
    {#each row as cell, j}
      {#if cell}
        <div
          class="flex flex-start absolute items-center justify-center border border-gray-300 bg-black bg-"
